<script>
    $(document).ready(function() {
        ...
        // --- AJAX Form Submit ---
        $('#transferUpdateForm').on('submit', function(e) {
            e.preventDefault();

            const formData = new FormData(this);
            const transferId = $('#transfer_id').val();
            const updateUrl = "{{ route('admin.transfer.update', ':id') }}".replace(':id', transferId);
            ...
            $.ajax({
                ...
                success: function(res) {
                    ...
                    if (res.status === 'success') {
                        showSuccessToast(res.message || 'Transfer updated successfully.');
                        setTimeout(() => {
                            window.location.href =
                                "{{ route('admin.all-transfer.item') }}";
                        }, 1500);
                    } else {
