 ...
 <input type="hidden" name="user_id" value="{{$users->id}}">
                                 ...
                                 <div class="col-md-6">
                                     ...
                                     <input type="emal" class="form-control" name="email" value="{{$users->email}}">
                                 </div>
    ...
    @foreach ($roles as $role)
        <option value="{{ $role->id }}" 
            {{ strtolower($users->role) === strtolower($role->name) ? 'selected' : '' }}>
            {{ $role->name }}
        </option>
    @endforeach
</select>
...
$(document).ready(function() {
    $('#submitUseradmin').on('submit', function(e) {
        ...
        let url = userId 
            ? "{{ route('admin.update.user', ':id') }}".replace(':id', userId) 
